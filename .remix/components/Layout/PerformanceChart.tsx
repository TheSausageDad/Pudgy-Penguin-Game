...
const DetailedPerformancePanelComponent: React.FC<DetailedPerformancePanelProps> = ({ 
  ...
  const fpsSparklineRef = useRef<HTMLCanvasElement>(null)
  const timingSparklineRef = useRef<HTMLCanvasElement>(null)
  const memorySparklineRef = useRef<HTMLCanvasElement>(null)
  ...
  useEffect(() => {
    if (fpsSparklineRef.current) {
      // Cap FPS values at 60 for consistent scaling and visibility of the 60fps line
      const cappedFpsData = sparklineData.map(d => Math.min(d.fps, 60))
      drawSparkline(fpsSparklineRef.current, cappedFpsData, 0, 60, '#22c55e')
    }
    if (timingSparklineRef.current) {
      drawSparkline(timingSparklineRef.current, sparklineData.map(d => d.frameTime), 0, 33.33, '#22c55e')
    }
    if (memorySparklineRef.current && sparklineData.some(d => d.memory)) {
      const memoryValues = sparklineData.filter(d => d.memory).map(d => d.memory.used)
      if (memoryValues.length > 0) {
        const maxMemory = Math.max(...memoryValues) * 1.2
        drawSparkline(memorySparklineRef.current, memoryValues, 0, maxMemory, '#22c55e')
      }
    }
